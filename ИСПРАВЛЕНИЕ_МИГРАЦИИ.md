...
⚠️ **САМЫЙ ПРОСТОЙ СПОСОБ:** Если данные не критичны, просто пересоздайте базу данных. См. файл `ПЕРЕСОЗДАНИЕ_БД.md`
...
Если таблица `Supervisor` уже создана, но содержит дубликаты, используйте Node.js скрипт:

```bash
# Если используется Docker
docker-compose -f docker-compose.prod.yml run --rm virtual-gallery npm run fix:migration

# Если без Docker
npm run fix:migration
```

**Альтернативный вариант:** Если у вас установлен sqlite3:
...
Или вручную через SQLite (если установлен):
